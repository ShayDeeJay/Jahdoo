...
public class AbilityWheelMenu extends Screen  {
    ...
    private List<String> getAllAbilities(){
        List<String> getKeysFromWand = new ArrayList<>();
        if(this.getMinecraft() == null) return getKeysFromWand;
        Player player = this.getMinecraft().player;
        if(player == null) return getKeysFromWand;

        player.getMainHandItem().get(DataComponentRegistry.WAND_ABILITY_HOLDER.get()).abilityProperties().forEach(
            (k,v) -> getKeysFromWand.add(k)
        );

        return getKeysFromWand;
    }

    @Override
    protected void init() {
        int predefinedPositions = getAllAbilities().size(); // Define the number of positions around the circle
        ...
        for (int i = 0; i < predefinedPositions; i++) {
            double angle = angleOffset + 2 * Math.PI * i / predefinedPositions; // Calculate angle for each position
            int buttonX = (int) (centerX + radius * Math.cos(angle)) - buttonSize / 2;
            int buttonY = (int) (centerY + radius * Math.sin(angle)) - buttonSize / 2;

            List<AbstractAbility> getCurrentAbility = AbilityRegister.getSpellsByTypeId(getAllAbilities().get(i));
                ...
                this.addRenderableWidget(
                    new ScreenButton(
                        ...
                        pButton -> {
                            ...
                            PacketDistributor.sendToServer(new SelectedAbilityC2SPacket(getAllAbilities().get(finalI)));
                        },
                        i
                    )
                );
...
//            ResourceLocation getAbilityId = TagHelper.getAbilityTypeWand(getMinecraft().player);
//            List<AbstractAbility> getAbility = AbilityRegister.getSpellsByTypeId(getAbilityId.getPath().intern());
//            CompoundTag compoundTag = getMinecraft().player.getMainHandItem().getOrCreateTag().getCompound(WAND_STORED_ABILITIES);
//            if(!getAbility.isEmpty()){
//                SharedUI.getAbilityNameWithColour(getAbility.get(0), guiGraphics, compoundTag, x, y - 90, true);
//            }
        }
    }
