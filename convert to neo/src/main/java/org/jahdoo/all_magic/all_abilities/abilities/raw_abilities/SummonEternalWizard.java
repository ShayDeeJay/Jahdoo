...
public class SummonEternalWizard extends DefaultEntityBehaviour {
    ...
    public void getAoeCloud(AoeCloud aoeCloud) {
        ...
        this.lifeTime = getTag().getModifierValue(LIFETIME);
    }

    @Override
    public TagModifierHelper getTag() {
        return new TagModifierHelper(aoeCloud.getwandabilityholder(), SummonEternalWizardAbility.abilityId.getPath().intern());
    }
...
//        BakedParticleOptions bakedParticle = new BakedParticleOptions(ElementRegistry.VITALITY.get().getTypeId(), 20, 3f, false);
        GeneralHelpers.getInnerRingOfRadiusRandom(aoeCloud.position(), 0.8, 5).forEach(
            positions -> GeneralHelpers
                .generalHelpers
                .sendParticles(serverLevel, new BakedParticleOptions(ElementRegistry.VITALITY.get().getTypeId(), 20, 3f, false), positions, 1, 0, 1,0,0.05)
        );
    }
...
//
//        GenericParticleOptions particleOptions = new GenericParticleOptions(ParticleStore.MAGIC_PARTICLE_SELECTION, 0, 0, 10, 0.1f, true);
